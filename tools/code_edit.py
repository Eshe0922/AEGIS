...
def process_diff(text:str, project_path: str):
    succeed_patches = ''
    fail_patches = ''
    edits = ''
    if contains_line_number(text): 
        fail_patches = "ERROR! Do not contain the line number in patch! Please remove the line numbers and keep the indentation correct."
        return fail_patches
    if not check_label_number(text):
        fail_patches =  f"""ERROR! Your change request contains incomplete patche(es).  Provide patches in following format:
###Thought: modify ...
###Action:                
{DIFF_FENCE[0]}
...
"""
    ...
    if len(edits) == 0:
        fail_patches =  f"""ERROR! No patch found that meets the format requirements. Provide patches in following format:
###Thought: modify ...
###Action:                
{DIFF_FENCE[0]}
/absolute/path/of/modified_file.py
...
if __name__ == "__main__":
    ...
    with open(args.patch_file, 'r') as file:
        content = file.read()
    print(process_diff(content, args.project_path))